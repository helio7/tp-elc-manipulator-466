...
void loop(){
  if(BT.available()){
    ...
    if (valor_recibido == 3 and analogRead(A0) < 435){ //-5
      Orden_Giro_neg();
      
      while(!BT.available()and analogRead(A0) < 435){
        Ciclo_10hz();
        ciclos_orden = ciclos_orden + 1;
        if (ciclos_orden == 50){
          ResetPins();
    ...
    if (valor_recibido == 4 and analogRead(A0) > 125){ //-5
      Orden_Giro_pos();
      
      while(!BT.available() and analogRead(A0) > 125){ //-5
        Ciclo_10hz();
        ciclos_orden = ciclos_orden + 1;
        if (ciclos_orden == 50){
          ResetPins();
